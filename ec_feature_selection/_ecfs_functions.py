...
def get_fisher_score(X, y, negative_class, positive_class, epsilon):
    ...
    sum_label_mu = np.zeros(X.shape[1])
    sum_label_var = np.zeros(X.shape[1])

    for label in y_labels:
        label_mu = X[y == label].mean(axis=0)
        label_var = X[y == label].var(axis=0)
        label_var[label_var == 0] = epsilon

        sum_label_var += label_var
        sum_label_mu += (label_mu - X.mean(axis=0))** 2
    ...
    fisher_score = sum_label_mu / sum_label_var
    return fisher_score
